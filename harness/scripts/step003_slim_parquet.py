...
- ACLD/ACLR: confirmed pair 27쌍만 (ACLR38→ACLR36 명시적 치환) / HA: 전원
...
def run():
    ...
    aclr_df = aclr_df[aclr_df["subject_id"].isin(paired_aclr)]
    ...
    assert set(acld_df["subject_id"].unique()) == set(paired_acld), "ACLD confirmed pair cohort 불일치"
    assert set(aclr_df["subject_id"].unique()) == set(paired_aclr), "ACLR confirmed pair cohort 불일치"
