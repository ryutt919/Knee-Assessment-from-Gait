...
- **상태**: v1 산출물은 legacy로 보존한다. v2는 두 balance mode 재현용 tag로 유지한다. v3.1은 mean-only session representation, scalar405 고정 primary, scalar+GVS54와 scalar+waveform5454 comparator, pair-aware repeated nested CV, HA LOO calibration과 direct D² contribution을 구현했다. 실제 36-session dry end-to-end 및 manifest 재현 검증을 통과했으며 full repeated run은 별도 실행이 필요하다.
...
| 2026-07-04 17:24 | Mahalanobis v3.1 mean-only scalar primary | scalar405 primary와 GVS54/waveform5454 fusion comparator, identity-balanced repeated nested CV, paired ΔAUC, strict side contract, HA side-swap sensitivity, version registry와 manifest 재현 구현; 실제 12 ACL/12 HA identity dry E2E 및 14 tests 통과 |
