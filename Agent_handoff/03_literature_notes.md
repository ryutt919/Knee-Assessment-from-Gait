...
### 2026-07-04 New Paper Corpus Ingest (15 papers, `01~07`)
- **Current value/logic**: `docs/ref_papers/00_upstream` held 18 newly supplied PDFs. SHA-256 dedup found 3 exact-duplicate pairs, leaving 15 unique papers. All 15 were renamed to their official titles, moved into `docs/ref_papers/01_*`–`07_*` (1/2/1/3/2/6 split across `01/03/04/05/06/07`), and the 3 duplicate copies were isolated (not deleted) under `docs/ref_papers/99_duplicates_or_alternate_versions/`. `docs/ref_papers/00_upstream` is now empty. Corpus total is 60 unique analyzed papers (45 pre-existing + 15 new); `docs/ref_papers/01_*`–`07_*` sum to 62 files including the 2 duplicate-source originals still counted at their canonical location.
- **Implementation**: Added `--input-manifest` to `run_pipeline.py`/`01_analyze_reference_papers.py`/`02_review_paper_summaries.py` to process an exact PDF set (verified by `sha256`) instead of directory globbing, extended category discovery from `01~06` to `01~07`, and fixed pipeline-level bugs: incremental runs now preserve the cumulative `inventory` list across manifests, the process exit code reflects only the selected batch's success/failure (not the full 60-paper corpus), and stale `provider_states` are reset only after a fresh `--provider-test-all` gate passes. Existing paper numbers were preserved; the 15 new papers were assigned the next sequential numbers after the prior max, and `02` (fact-check) reused the same numbers from `01`'s manifest.
- **Provider gate reality check**: `codex` was rate-limited (`usage limit`, resets ~04:58) for this run. `claude` is structurally unusable as a CLI provider in this sandboxed dev environment for two independent reasons: (1) its 1M-context default model requires `usage credits` not enabled on this account (`API Error: Usage credits required for 1M context`), and (2) even with `--claude-model` pinned to a standard-context model, invoking `claude --print` from inside a running Claude Code session spins up a full nested agent (hooks, plan mode, tool list) that treats the provider-test's "return this JSON verbatim, no tools" instruction as a suspicious prompt injection and refuses. Proceeded with `antigravity` as the sole working provider (`--skip-provider-gate`) rather than blocking on codex/claude; also had to add `/opt/homebrew/bin` to `PATH` in the exec shell for `pdftotext`/`claude`/`codex` binaries to resolve at all. All 15 papers succeeded on `antigravity`; 3 papers hit a transient `invalid_output` (JSON not found in response) on the first pass and succeeded on `--retry-failed` without code changes.
- **Date-folder snapshot**: `Study-review/mds/2026-07-04/01_batch_manifest.json` and `02_batch_index.md` record the 15-paper batch (original filename, SHA-256, official title, final path, category). `Study-review/mds/2026-07-04/papers/{category}/` and `.../reviews/{category}/` hold byte-identical copies (hash-verified) of the 15 analysis MDs and 15 fact-check MDs; canonical outputs remain under `Study-review/mds/papers/{category}` and `Study-review/mds/reviews/{category}`.
- **Related files**:
  - `Study-review/run_pipeline.py`, `Study-review/scripts/01_analyze_reference_papers.py`, `Study-review/scripts/02_review_paper_summaries.py`, `Study-review/scripts/path_utils.py`
  - `Study-review/tests/test_01_reference_pipeline.py` (added coverage for `07` category, manifest-based selection, number stability, cumulative inventory, per-selection exit codes, provider-state reset)
  - `Study-review/mds/2026-07-04/` (batch manifest, index, snapshot copies)
  - `docs/ref_papers/99_duplicates_or_alternate_versions/` (3 isolated duplicate PDFs)
- **Rationale**: The user supplied 18 new PDFs and wanted them deduplicated, titled, filed into the existing taxonomy, and run through the same analyze→fact-check pipeline as the prior 45 papers, with a per-batch dated snapshot alongside the canonical category-organized output — without disturbing prior paper numbers, unrelated in-flight work (Mahalanobis pipeline, `.omc` state), or requiring manual revision/claims regeneration/Notion upload for this pass.
...
| 2026-07-04 02:05 | Ingest 15 new papers (18 upstream, 3 dedup) | 45-paper corpus, `01~06` scope, directory-glob only -> 60-paper corpus, `01~07` scope, `--input-manifest` exact-set selection added; 15 new papers analyzed+fact-checked via `antigravity` (codex rate-limited, claude structurally blocked in this sandbox) with 3 transient retries; dated snapshot written to `Study-review/mds/2026-07-04/` |
