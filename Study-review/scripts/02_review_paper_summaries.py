...
def main() -> int:
    ...
    for idx, target in enumerate(targets, 1):
        key = str(target.pdf.resolve())
        category = category_for_pdf(target.pdf)
        ...
        for provider in provider_order(args.provider):
            ...
            if attempt.ok and attempt.data is not None:
                ...
                filename = f"{target.number:02d}_{safe_name(target.pdf)}_review.md"
                out_path = categorized_output_path(output_root, "reviews", target.pdf, filename)
