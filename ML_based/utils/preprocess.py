...
def get_stance_segments(contact_signal: np.ndarray, max_gap: int = 8) -> list:
    """contact(0/1) 신호에서 연속된 stance 구간 [start, end) 목록을 반환한다.

    max_gap: 이 샘플 수 이하의 0 구간은 같은 stance로 합쳐 처리.
             heel→toe 전환 공백(IMU 데이터에서 최대 8샘플 관측) 보정용.
    """
    ...
    for idx, val in enumerate(contact_binary):
        if val == 1 and seg_start is None:
            ...
            raw.append((seg_start, idx))
            seg_start = None

    if seg_start is not None:
        raw.append((seg_start, len(contact_binary)))

    if max_gap <= 0 or len(raw) < 2:
        return raw
